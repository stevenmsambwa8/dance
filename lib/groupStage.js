 ...
 */
export function buildGroups(participants, groupCount, teamSize = 1, legs = 1) {
  ...
  const groups = Array.from({ length: groupCount }, (_, i) => ({
    id: `group_${i}`,
    name: groupCount === 1 ? 'League Table' : `Group ${String.fromCharCode(65 + i)}`, // A, B, C...
    members: [],
  }))
  ...
  return groups.map(gr => ({
    ...gr,
    fixtures: generateRoundRobinFixtures(gr.members, legs),
  }))
}
 ...
 *
 * `legs`: 1 = single round (each pair meets once). 2 = home & away
 * (each pair meets twice, with home/away reversed the second time) —
 * used by the Premier-League-style format where the whole tournament
 * is one all-play-all table instead of a bracket.
 */
export function generateRoundRobinFixtures(members, legs = 1) {
  ...
  if (legs >= 2) {
    // Second leg — same pairings, home/away reversed, appended after the
    // first leg's rounds so the standings engine (which is leg-agnostic)
    // just sees more fixtures between the same two ids.
    const secondLeg = fixtures.map(fx => ({
      id: `${fx.awayId}_vs_${fx.homeId}_r${fx.round + rounds}`,
      round: fx.round + rounds,
      homeId: fx.awayId,
      awayId: fx.homeId,
      scoreHome: null,
      scoreAway: null,
...
export function computeStandings(group) {
  ...
  group.members.forEach(m => {
    ...
    table[id] = {
      id, name: m.name, avatar: m.avatar || null, removed: !!m.removed,
  ...
  // Manual admin adjustments (no-show penalties, corrections) layered on top
  // of the fixture-derived points — stored per-group as { [memberId]: number }
  // so they persist through recompute and show up in the table's Pts column
  // (previously these only lived in tournament_leaderboard, invisible here).
  const adjustments = group.adjustments || {}
  Object.entries(adjustments).forEach(([mid, delta]) => {
    if (table[mid] && delta) table[mid].points += delta
  })

  const rows = Object.values(table)
    // A removed player's own row disappears from the table entirely (that's
    // the "remove the actual one" the organiser wants) — but note this only
    // hides their ROW; the fixture loop above already ran, so anything a
    // remaining player earned by beating them stays counted in that
    // player's points/goal-difference. Nothing gets taken away retroactively.
    .filter(r => !r.removed)
    .map(r => ({ ...r, goalDiff: r.goalsFor - r.goalsAgainst, adjustment: adjustments[r.id] || 0 }))
...
export function isGroupComplete(group) {
  // 'void' fixtures are ones the admin explicitly marked as "not played" (no
  // points either way) — they're a settled outcome, not a pending one, so
  // they shouldn't block the group/league from ever auto-advancing.
  return group.fixtures.every(fx => fx.status === 'played' || fx.status === 'void')
}
...
export function addMemberToGroup(group, member, legs = 1) {
  ...
  const newFixtures = group.members.flatMap(m => {
    ...
    if (legs < 2) return [leg1]
    const leg2 = {
      id: `${memberId}_vs_${existingId}_join${stamp}b`,
      round: 1,
      homeId: memberId,
      awayId: existingId,
      scoreHome: null,

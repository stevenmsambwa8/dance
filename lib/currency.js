/**
 * currency.js
 * Africa currency utilities for Nabogaming.
 * Base currency: TZS — all prices stored in DB as TZS.
 * Supported display currencies: TZS, KES, UGX, ZAR, NGN
 *
...
export const CURRENCIES = {
  ...
  ZAR: { code: 'ZAR', label: 'South African Rand', symbol: 'ZAR', flag: 'south-africa', locale: 'en-ZA' },
  NGN: { code: 'NGN', label: 'Nigerian Naira',      symbol: '₦',   flag: 'nigeria',      locale: 'en-NG' },
}
  ...
  _fetchPromise = (async () => {
    try {
      ...
      _ratesCache = {
        ...
        ZAR: json.rates?.ZAR ?? FALLBACK_RATES.ZAR,
        NGN: json.rates?.NGN ?? FALLBACK_RATES.NGN,
      }
      return _ratesCache
    } catch {
...
export function currencyFromFlag(flag) {
  ...
  if (flag === 'south-africa') return 'ZAR'
  if (flag === 'nigeria')      return 'NGN'
  return 'TZS'
}

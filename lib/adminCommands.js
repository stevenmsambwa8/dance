...
async function editTournament(supabase, { tournament_id, name, prize, format, slots, date, description, status, entrance_fee }) {
  ...
  if (slots !== undefined && slots !== '') updates.slots = Number(slots);
  if (date !== undefined) updates.date = date;
  if (description !== undefined) updates.description = description;
  if (status !== undefined) updates.status = status;
  if (entrance_fee !== undefined) updates.entrance_fee = entrance_fee ? Number(String(entrance_fee).replace(/,/g, '')) : 0;
  if (Object.keys(updates).length === 0) throw new Error('Nothing to update');

  const { data, error } = await supabase.from('tournaments').update(updates).eq('id', tournament_id).select().single();
  if (error) throw error;
  return { tournament: data };
}

// Returns the full deleted row as `deleted` so the caller (the worker page)
// can keep a client-side history/undo entry — no new DB table needed.
async function deleteTournament(supabase, { tournament_id }) {
  if (!tournament_id) throw new Error('tournament_id is required');
  const tournament = await getTournamentOrThrow(supabase, tournament_id);
  await supabase.from('tournament_leaderboard').delete().eq('tournament_id', tournament_id);
  await supabase.from('tournament_participants').delete().eq('tournament_id', tournament_id);
  const { error } = await supabase.from('tournaments').delete().eq('id', tournament_id);
  if (error) throw error;
  return { deleted: tournament };
}

// Clones a tournament's settings into a fresh one. No participants or
// bracket progress carried over, so it's safe to fire immediately.
async function duplicateTournament(supabase, { tournament_id }) {
  if (!tournament_id) throw new Error('tournament_id is required');
  const t = await getTournamentOrThrow(supabase, tournament_id);
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      entrance_fee: t.entrance_fee || 0,
      team_size: t.team_size || 1,
      stage_format: t.stage_format || 'groups_knockout',
      group_count: t.group_count ?? null,
      advance_per_group: t.advance_per_group ?? null,
      bracket_data: null,
      round_names: null,
      is_test: t.is_test || false,
      pro_only: t.pro_only || false,
      clan_id: t.clan_id || null,
      status: 'active',
      registered_count: 0,
...
export const COMMANDS = {
  ...
  'duplicate-tournament': duplicateTournament,

...
function gameImage(slug) {
  return GAME_META[slug]?.image || null
}
...
async function completedMatchStories(limit) {
  const { data } = await supabase
    ...
    .select(`
      ...
      challenger:profiles!matches_challenger_id_fkey(id, username, avatar_url),
      challenged:profiles!matches_challenged_id_fkey(id, username, avatar_url)
    `)
    .eq('status', 'completed')
  ...
  return (data || []).map(m => {
    const winnerIsChallenger = m.winner_id === m.challenger?.id
    const winnerName = winnerIsChallenger ? m.challenger?.username
                      : m.winner_id === m.challenged?.id ? m.challenged?.username
                      : null
    const loserName  = winnerIsChallenger ? m.challenged?.username : m.challenger?.username
    ...
    // Two players mentioned in this headline → combine both avatars,
    // winner shown first/on top.
    const winnerAvatar = winnerIsChallenger ? m.challenger?.avatar_url : m.challenged?.avatar_url
    const loserAvatar  = winnerIsChallenger ? m.challenged?.avatar_url : m.challenger?.avatar_url
    ...
    return {
      ...
      href:     `/matches/${m.id}`,
      icon:     'ri-sword-line',
      media:    { kind: 'duo', a: winnerAvatar, b: loserAvatar },
      at:       m.created_at,
    }
  }).filter(Boolean)
}
...
async function tournamentStories(limit) {
  ...
  return (data || []).map(t => {
    ...
    return {
      ...
      icon:     'ri-trophy-line',
      media:    { kind: 'game', src: gameImage(t.game_slug) },
      at:       t.created_at,
    }
  })
}
...
async function gameChatStories(limit) {
  ...
  for (const msg of data || []) {
    ...
    stories.push({
      ...
      media:    { kind: 'game', src: gameImage(msg.game_slug) },
      at:       msg.created_at,
    })
...
async function feedPostStories(limit) {
  const { data } = await supabase
    .from('posts')
    .select('id, content, likes, comment_count, created_at, profiles(username, avatar_url)')
    .order('created_at', { ascending: false })
    .limit(limit)
  ...
  return (data || []).map(p => ({
    ...
    media:    { kind: 'avatar', src: p.profiles?.avatar_url || null },
    at:       p.created_at,
  }))
}

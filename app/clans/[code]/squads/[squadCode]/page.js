...
export default function SquadPage() {
  ...
  async function loadAll() {
    ...
    const { data: memberData } = await supabase.from('clan_members')
      .select('*, profiles(id, username, avatar_url, tier, level, email, plan, plan_expires_at, country_flag, is_season_winner, custom_badges, temp_admin_until)')
              ...
              <div className={styles.memberCardContent}>
                <div className={styles.memberNameRow}>
                  ...
                  <UserBadges
                    ...
                    countryFlag={m.profiles?.country_flag} isSeasonWinner={m.profiles?.is_season_winner} customBadges={m.profiles?.custom_badges} tempAdminUntil={m.profiles?.temp_admin_until}
                    size={11} gap={2}/>
                </div>

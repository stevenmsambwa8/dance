...
export default function SeasonPage() {
  ...
  if (!user) return (
    <div className={styles.page}>
      <div className={styles.guestWrap}>
        ...
        <button onClick={openAuthGate} className={styles.ctaBtn}>Sign In</button>
      </div>
    </div>
  )
  ...
  const prizeTZS   = logs.filter(l => l.type === 'prize' || l.type === 'join_bonus' || l.type === 'full_bonus' || l.type === 'referral_bonus').reduce((s, l) => s + (l.points ?? 0), 0)
        ...
        <div className={styles.headerTop}>
          <div className={styles.headerLeft}>
            ...
            <h1 className={styles.title}>
              ...
              <UserBadges email={profile?.email} plan={profile?.plan} planExpiresAt={profile?.plan_expires_at} countryFlag={profile?.country_flag} isSeasonWinner={profile?.is_season_winner} customBadges={profile?.custom_badges} tempAdminUntil={profile?.temp_admin_until} size={16} gap={4} />
            </h1>
          </div>

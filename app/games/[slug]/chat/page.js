    ...
    const ch = supabase.channel('gchat-' + slug)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'game_chat_messages', filter: 'game_slug=eq.' + slug }, payload => {
        supabase.from('game_chat_messages')
          .select('*, profiles(id, username, avatar_url, email, tier, level, country_flag, is_season_winner, custom_badges, temp_admin_until, plan, plan_expires_at)')
  ...
  async function loadAll() {
    ...
    const [{ data: msgs }, { count: subs }, profResult, subResult] = await Promise.all([
      ...
      user ? supabase.from('profiles')
        .select('id, username, avatar_url, email, tier, level, country_flag, is_season_winner, custom_badges, temp_admin_until')
  ...
  if (!user) return (
    <div className={styles.page}>
      <div className={styles.centered}>
        ...
        <button onClick={openAuthGate} className={styles.primaryBtn}>Log In</button>
      </div>
    </div>
  )
  if (loading) return (
                        ...
                        >
                          ...
                          {!mine && isFirst && !isSpecial && (
                            <div className={styles.bubbleSender}>
                              ...
                              <UserBadges email={msg.profiles?.email} plan={msg.profiles?.plan} planExpiresAt={msg.profiles?.plan_expires_at} countryFlag={msg.profiles?.country_flag} isSeasonWinner={msg.profiles?.is_season_winner} customBadges={msg.profiles?.custom_badges} tempAdminUntil={msg.profiles?.temp_admin_until} size={10} gap={2} />

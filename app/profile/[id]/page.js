    ...
    <div className={styles.page}>
      ...
      <div className={styles.helpdeskCard}>
        ...
        <button
          ...
          onClick={() => user ? router.push('/help-desk') : openAuthGate()}
        >
                      ...
                      <Link key={item.id} href={`/shop/${item.id}`} className={styles.shopCard}>
                        ...
                        <div className={styles.shopCardBody}>
                          ...
                          <span className={styles.shopPrice}>
                            {fmtAmt(Number(String(item.price).replace(/,/g, ''))) /* converted */}
                            {false && isNaN(Number(String(item.price).replace(/,/g, '')))
                              ? item.price
        ...
        footer={
          user ? (
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: '0.8rem' }}>
              <button onClick={openAuthGate} style={{background:'none',border:'none',color:'var(--text)',fontWeight:700,cursor:'pointer',padding:0,textDecoration:'underline',fontFamily:'var(--font)'}}>Log in</button> to comment
            </p>
          )
        }
      >

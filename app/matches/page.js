...
export default function Matches() {
  ...
  async function loadLiveMatches() {
    ...
    let q = supabase
      .from('matches')
      .select('*, challenger:profiles!matches_challenger_id_fkey(username, tier, level, avatar_url, email, country_flag, is_season_winner, custom_badges, temp_admin_until, plan, plan_expires_at), challenged:profiles!matches_challenged_id_fkey(username, tier, level, avatar_url, email, country_flag, is_season_winner, custom_badges, temp_admin_until, plan, plan_expires_at)')
                    ...
                    <div className={styles.content}>
                      <div className={styles.players}>
                        <span className={styles.pname}>
                          ...
                          <UserBadges email={m.challenger?.email} plan={m.challenger?.plan} planExpiresAt={m.challenger?.plan_expires_at} countryFlag={m.challenger?.country_flag} isSeasonWinner={m.challenger?.is_season_winner} customBadges={m.challenger?.custom_badges} tempAdminUntil={m.challenger?.temp_admin_until} size={11} gap={2} />
                        </span>
                        <span className={styles.vs}>vs</span>
                        <span className={styles.pname}>
                          {isRecruiting ? <em style={{ color: 'var(--text-muted)', fontStyle: 'normal' }}>Open slot</em> : (m.challenged?.username || '—')}
                          {!isRecruiting && <UserBadges email={m.challenged?.email} plan={m.challenged?.plan} planExpiresAt={m.challenged?.plan_expires_at} countryFlag={m.challenged?.country_flag} isSeasonWinner={m.challenged?.is_season_winner} customBadges={m.challenged?.custom_badges} tempAdminUntil={m.challenged?.temp_admin_until} size={11} gap={2} />}
                        </span>
                      </div>
                  ...
                  <div className={styles.content}>
                    <div className={styles.players}>
                      ...
                      <span className={`${styles.pname} ${cdWon ? styles.pnameWon : ''} ${isCompleted && !cdWon && m.winner_id ? styles.pnameLost : ''}`}>
                        ...
                        <UserBadges email={m.challenged?.email} plan={m.challenged?.plan} planExpiresAt={m.challenged?.plan_expires_at} countryFlag={m.challenged?.country_flag} isSeasonWinner={m.challenged?.is_season_winner} customBadges={m.challenged?.custom_badges} tempAdminUntil={m.challenged?.temp_admin_until} size={11} gap={2} />
                      </span>
